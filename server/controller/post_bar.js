...
exports.publishList = (req, res) => {
	const sql = `
  SELECT a.ID,a.title,a.details,a.add_time,b.nickname,b.head_thumb,c.count,d.theme
	FROM post_bar AS a 
...
exports.typeList = (req, res) => {
  ...
  const sql = `
  SELECT a.ID,a.title,a.details,a.add_time,b.nickname,b.head_thumb,c.count
  FROM post_bar AS a 
  LEFT JOIN (SELECT post_bar_id,COUNT(*) AS count FROM reply GROUP BY post_bar_id) AS c ON a.ID = c.post_bar_id
  INNER JOIN user AS b ON a.user_id = b.ID
  INNER JOIN theme AS d ON a.theme_id = d.ID
...
exports.publish = (req, res) => {
    ...
    (title, details, add_time, user_id, theme_id)
    VALUES (?, ?, ?, ?, ?)
    `;
		database.query(queryStr, [title, details, addTime, userID, themeID], (data) => {
